...
class Level7ViewController: UIViewController {
    ...
    private var segmentedControls: [UISegmentedControl] = []
    private var circleIndicators: [UIView] = []
    private var audioPlayer: AVAudioPlayer?
    private var levelLabel: UILabel!
    private var targetSelections: [Int] = [1, 0, 2]
    ...
    private func setupUI() {
        ...
        levelLabel.text = LocalizationManager.Levels.level7.localized
    ...
    @objc private func segmentedControlChanged() {
        playSound()
        let currentSelections = segmentedControls.map { $0.selectedSegmentIndex }
        
        let currentSuccessCount = zip(currentSelections, targetSelections)
            .filter { $0 == $1 }
            .count
        ...
        if currentSuccessCount == targetSelections.count {
            animateSuccess()
        }
    }
    ...
    private func updateProgress(successCount: Int) {
        let progress = Float(successCount) / Float(targetSelections.count)
        UIView.animate(withDuration: 0.3) {
    ...
    private func animateSuccess() {
        UIView.animate(withDuration: 0.3, animations: {
            ...
            UIView.animate(withDuration: 0.2) {
                ...
                let level8VC = Level8ViewController()
                level8VC.modalPresentationStyle = .fullScreen
                self.present(level8VC, animated: true)
            }
        }
    }
